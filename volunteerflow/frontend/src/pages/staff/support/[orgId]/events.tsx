...
export default function SupportEventsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    staffApi.get(`/orgs/${orgId}/events`)
      .then(data => setEvents((data as { events: OrgEvent[] }).events ?? []))
  ...
  return (
    <SupportViewLayout orgId={orgId} activeHref={`/staff/support/${orgId}/events`}>
          ...
          <table className="w-full text-sm text-gray-300">
            ...
            <tbody className="bg-gray-900 divide-y divide-gray-800">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-gray-500">No events found.</td>
                </tr>
              ) : (
                events.map(e => (
                  <tr key={e.id} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-400">{e.location || '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                        e.status === 'published' ? 'bg-green-900/40 text-green-300' :
                        e.status === 'draft' ? 'bg-gray-700 text-gray-400' :
                        e.status === 'cancelled' ? 'bg-red-900/40 text-red-300' :
